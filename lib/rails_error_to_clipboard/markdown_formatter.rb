...
module RailsErrorToClipboard
  class MarkdownFormatter
    ...
    def exception_class
      return "Unknown" if @exception.nil?

      @exception.class.name
    end

    def exception_message
      return "No exception" if @exception.nil?
    ...
    def request_params
      ...
      params = @request.params.reject { |k, _| %w[controller action].include?(k.to_s) }
      if params.empty?
        "- **Params:** {}"
      else
    ...
    def formatted_backtrace
      return "No backtrace available" if @exception.nil? || @exception.backtrace.nil?
